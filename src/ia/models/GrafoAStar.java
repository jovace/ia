...
import ia.exceptions.NoSuchElementException;

public final class GrafoAStar<T> implements Iterable<T> {
    /*
     * A map from the nodeId to outgoing edge.
     * An outgoing edge is represented as a tuple of NodeData and the edge length
     */
    private final Map<T, Map<Nodo<T>, Conexion>> graph;
    /*
     * A map of heuristic from a node to each other node in the graph.
     */
    private final Map<T, Map<T, Conexion>> heuristicMap;
    /*
     * A map between nodeId and Nodo.
     */
    private final Map<T, Nodo<T>> nodeIdNodeData;

    public GrafoAStar(Map<T, Map<T, Conexion>> heuristicMap) {
        if (heuristicMap == null) throw new NullPointerException("The huerisic map should not be null");
        graph = new HashMap<T, Map<Nodo<T>, Conexion>>();
        nodeIdNodeData = new HashMap<T, Nodo<T>>();
     ...
     */
    public void addNode(T nodeId) throws NoSuchElementException {
        ...
        graph.put(nodeId, new HashMap<Nodo<T>, Conexion>());
        nodeIdNodeData.put(nodeId, new Nodo<T>(nodeId, heuristicMap.get(nodeId)));
    }
     ...
     * @throws NoSuchElementException 
     */
    public void addEdge(T nodeIdFirst, T nodeIdSecond, double length) throws NoSuchElementException {
        ...
        graph.get(nodeIdFirst).put(nodeIdNodeData.get(nodeIdSecond), new Conexion(length));
        graph.get(nodeIdSecond).put(nodeIdNodeData.get(nodeIdFirst), new Conexion(length));
    }
     ...
     * @throws NoSuchElementException 
     */
    public Map<Nodo<T>, Conexion> edgesFrom (T nodeId) throws NoSuchElementException {
     ...
     * @throws NoSuchElementException 
     */ 
    public Nodo<T> getNodeData (T nodeId) throws NoSuchElementException {
